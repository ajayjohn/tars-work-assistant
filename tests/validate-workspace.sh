...
grep -q "from the user's workspace" commands/welcome.md && pass "welcome fallback avoids workspace skill lookup" || fail "welcome fallback may read plugin skill from workspace"
grep -q "Slash commands are optional" commands/welcome.md && grep -q "Natural-language example" commands/welcome.md && pass "welcome fallback includes natural-language command examples" || fail "welcome fallback lacks natural-language command examples"
grep -q "Paste or upload a meeting transcript" commands/welcome.md && pass "welcome fallback prescribes first demo" || fail "welcome fallback first demo missing"
grep -q "Do not recommend \`/briefing\` as a starter action" commands/welcome.md && pass "welcome fallback avoids empty-workspace briefing suggestion" || fail "welcome fallback may suggest briefing too early"
grep -q "Do not recommend \`/briefing\` as a starter action" skills/welcome/SKILL.md && pass "welcome skill avoids empty-workspace briefing suggestion" || fail "welcome skill may suggest briefing too early"
