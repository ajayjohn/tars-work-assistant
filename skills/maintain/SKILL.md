...
help:
  purpose: |
    Comprehensive workspace maintenance with four distinct modes: running health checks on naming and frontmatter, syncing tasks and detecting memory gaps, rebuilding all index files, and batch-processing inbox items with parallel sub-agents.
  use_cases:
    - "When you need to run workspace housekeeping checks for naming violations and broken links"
    - "When you want to sync tasks from integrations and find memory gaps"
    - "When your indexes are stale or corrupted and need regeneration"
    - "When you have pending items in your inbox ready for processing"
  invoke_examples:
    - natural: "housekeeping"
    - natural: "sync tasks"
    - natural: "rebuild indexes"
    - natural: "process inbox"
    - slash: "/maintain health"
    - slash: "/maintain sync"
    - slash: "/maintain rebuild"
    - slash: "/maintain inbox"
  common_questions:
    - q: "How often should I rebuild indexes?"
      a: "Only when indexes are known to be stale or corrupted. Health checks run automatically at session start."
    - q: "What happens to items in the inbox when processed?"
      a: "Inbox items are classified, processed by sub-agents (creating tasks and memory entries), then moved to completed/ or failed/."
  related_skills: [welcome, tasks, learn]
---
