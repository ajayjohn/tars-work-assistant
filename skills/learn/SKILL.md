...
help:
  purpose: |
    Two complementary knowledge management modes. Memory mode extracts high-signal, durable insights from conversations and persists them to the knowledge graph after passing the durability test. Wisdom mode processes learning content (articles, podcasts, videos, papers) into structured reports with key insights, mental models, and actionable takeaways.
  use_cases:
    - "When user wants to persist a fact or insight to memory"
    - "When user says 'remember this' or 'save to memory'"
    - "When user wants to extract wisdom or insights from learning content"
    - "When user says 'extract wisdom from this article' or 'process this podcast'"
    - "When user shares organizational context that should be remembered"
  invoke_examples:
    - natural: "Remember this—Sarah now reports to Mike"
    - natural: "Save this to memory"
    - natural: "Extract wisdom from this article"
    - natural: "Process this podcast transcript"
    - slash: "/learn memory"
    - slash: "/learn wisdom"
  common_questions:
    - q: "Does everything get saved to memory?"
      a: "No. The durability test requires four criteria to pass: lookup value, high signal, durability, and behavior change. If any fails, the insight does NOT persist."
    - q: "What is 'wisdom extraction'?"
      a: "Wisdom mode processes learning content (not meetings) to extract profound insights, mental models, frameworks, and actionable advice. It distinguishes between conversational sources (podcasts, blogs) and authoritative sources (research papers, documentation)."
    - q: "Will this also create tasks?"
      a: "In wisdom mode, yes—if the content surfaces actionable items or research needs. In memory mode, no."
  related_skills: [meeting, tasks, maintain]
---
