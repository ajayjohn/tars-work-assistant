---
name: meeting
description: Process meeting transcripts to extract reports, tasks, and memory with calendar integration
user-invocable: true
help:
  purpose: |
    Processes meeting transcripts into structured reports with topics, updates, concerns, decisions, and action items. Extracts actionable tasks with accountability validation, persists durable insights to memory using the durability test, and saves journal entries for reference.
  use_cases:
    - "When user wants to process a meeting transcript"
    - "When user has a meeting recording or transcript to analyze"
    - "When user says 'here's a transcript'"
    - "When meeting insights need to be extracted to memory and tasks"
  invoke_examples:
    - natural: "Process this meeting transcript"
    - natural: "Here's the meeting recording transcript—extract tasks and update memory"
    - slash: "/meeting <paste transcript>"
  common_questions:
    - q: "Will this create tasks automatically?"
      a: "Yes. After saving the journal entry, a parallel sub-agent extracts tasks and applies the accountability test. No permission needed."
    - q: "What if I don't have calendar integration configured?"
      a: "The skill works with transcript-only processing. It notes the gap but proceeds with extraction."
    - q: "How are names normalized?"
      a: "The skill reads reference/replacements.md and applies canonical names to all references before creating tasks and memory."
